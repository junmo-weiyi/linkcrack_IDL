...
import torch.nn.functional as F
#学习上采样+FEM(深度可分离卷积)+Downsample下采样(优化)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, out_channels,  BN_enable=True):
        ...
        self.up = LearnableUpsample(mid_channels,mid_channels)
...
class LinkCrack(nn.Module):
    ...
    def __init__(self):
        ...
        self.res3 = nn.Sequential(
            ...
            ResidualBlock(128, 128),  # 普通卷积块
            FEM(in_planes=128, out_planes=128),
            ResidualBlock(128, 128, dilation=(2,2)), 
            FEM(in_planes=128, out_planes=128),
            ResidualBlock(128, 128, dilation=(2,2)),
        )
